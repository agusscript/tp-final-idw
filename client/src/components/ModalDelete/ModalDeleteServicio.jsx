...
function ModalDeleteServicio({ show, item, setShow, setUpdateItems }) {
  function handleDelete(item) {
    try {
      fetch(`http://localhost:3001/servicio/deleteServicio/${item.idServicio}`, {
        method: 'DELETE'
      });
      window.alert("Servicio eliminado con exito");
    } catch (error) {
      window.alert("No se pudo eliminar el Servicio", error);
      throw new (error);
    } finally {
